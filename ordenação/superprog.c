...
r = strcmp(al[aux].nome,al[j].nome);
		  if(r > 0)	{
		strcpy(auxi,al[i].nome);
     	strcpy(al[i].nome,al[j].nome);
    	strcpy(al[j].nome,auxi);
	
    	temp[i]=al[i].matri;
	    al[i].matri=al[j].matri;
	    al[j].matri=temp[i];
...
for(j=i+1; j < tam; j++ ){
	...
	temp[i]=al[aux].matri;
	al[aux].matri=al[j].matri;
	al[j].matri=temp[i];
	
	strcpy(auxi,al[aux].nome);
	strcpy(al[aux].nome,al[j].nome);
	strcpy(al[j].nome,auxi);
